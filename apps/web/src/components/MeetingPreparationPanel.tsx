...
const EMPTY_PROGRESS: Stage2PrepareProgress = {
  frameworkJob: null,
...
export function MeetingPreparationPanel() {
  ...
  const [prepareProgress, setPrepareProgress] = useState<Stage2PrepareProgress>(EMPTY_PROGRESS);
  ...
  useEffect(() => {
    ...
    void runPrepare();
  }, [accessToken, opportunityId, draft, runPrepare]);
  ...
  const preparing = phase === "loading_context" || phase === "preparing" || phase === "redirecting";
      ...
      <main className="app-shell app-workspace-body">
        ...
        {notice ? (
          <RecoveryBanner
            ...
            onAction={() => {
              if (
                notice.action?.kind === "GENERATE" ||
                notice.action?.kind === "RETRY" ||
