"""Seed the in-memory API with MS-30 fixture client records for FIGMA-04 validation."""
...
API_BASE = os.getenv("FIGMA04_API_BASE", "http://127.0.0.1:8000").rstrip("/")
...
def main() -> int:
    token = os.getenv("FIGMA04_ACCESS_TOKEN", "").strip()
    if not token:
        print(
            "FIGMA04_ACCESS_TOKEN is required (use the local dev bearer token; never commit it).",
            file=sys.stderr,
        )
        return 2
    ...
    bare = manifest["bare_client"]
    contact = rich_pack["contacts"][0]

    rich_opportunity = _request(
        token,
        "POST",
        "/opportunities",
        {
    ...
    bare_opportunity = _request(
        token,
        "POST",
        "/opportunities",
        {
            "client_name": bare["name"],
            "opportunity_name": bare["opportunity"],
            "department": bare["department"],
    ...
    _request(
        token,
        "POST",
        f"/opportunities/{rich_opportunity['id']}/transcripts",
        multipart=("meeting.txt", b"DEMO DATA - post-meeting transcript excerpt.", "text/plain"),
    )

    print(
        json.dumps(
            {
                "rich_opportunity_id": rich_opportunity["id"],
                "bare_opportunity_id": bare_opportunity["id"],
                "clients": [rich["name"], bare["name"]],
            }
        )
    )
    return 0
